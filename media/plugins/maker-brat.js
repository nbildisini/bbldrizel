...
module.exports = {
    ...
    exec: async (m, from, { q, fuzzy, args, command, prefix, reply }) => { // START
        ...
        const clairityApi = `https://api.siputzx.my.id/api/m/brat?text=${encodeURIComponent(q.trim())}&isVideo=false&delay=500`;
        const siputApi = `https://aqul-brat.hf.space/?text=${encodeURIComponent(q.trim())}`;
